...
import {getLeaveRequest, listLeaveRequests, updateLeaveRequest} from "../../requests/leave-requests";
...
const LeaveManagement = () => {
	...
	const leaveRequestTableColumn = [
		{
			...
			dataIndex: 'employee',
			render: (text, record) => {
				return <Link to={`/employees/${record.employee.id}`}>
		...
		},
		{
			...
			render: (text, record) => {
				if (record.reviewed_by) {
					return <Link to={`/employees/${record.reviewed_by.id}`}>
						{record?.reviewed_by?.first_name} {record?.reviewed_by?.last_name}
					</Link>
				}
			}
		},
	...
	useEffect(() => {
		const fetchLeaveRequests = async () => {
			try {
				const response = await listLeaveRequests(tokens.access);
				setLeaves(response)
			} catch (e) {
				setLeaves([])
			}
